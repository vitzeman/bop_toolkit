...
class AppWindow:
    ...
    def _transform(self, event):
        ...
        def move(x, y, z, rx, ry, rz):
            ...
            # T = active_obj.transform
            # R = T[0:3, 0:3].tolist()
            # t = T[0:3, 3].tolist()
            # dict = {"cam_R_m2c": R, "cam_t_m2c": t}
            # print(dict)
            # print(active_obj.transform)

        if event.type == gui.KeyEvent.DOWN:  # only move objects with down strokes
            # Refine
            if event.key == gui.KeyName.R:
                print("R pressed: Refine object")
                self._on_refine()
            # Translation
